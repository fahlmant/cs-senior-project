...
\hline
    ...
    \hline
    OpenGL Deliverable & Project shifted from OpenGL to WebGL & Client requested that we work directly on project. \\
    Proposal of Software & Discussed informally with client and made decision without a write-up & Client changed software without needing a formal proposal\\
    Use of CloudCompare & Client requested a move to Potree due to time and ease of use & We wish we had made the move sooner.\\
    Rotate, crop, etc. for data & Not available in Potree & This is something that would need to be added to Potree first.
...
we chose, OSVR, was a constant throughout the whole project; the most core part of our project remained the same,
which made it much easier to start again with a new rendering software. The UI framework we chose was related
...
choice of peripheral hardware did not change, we did not reach a point in our project where we could use them, so
...
This project is capable of rendering point clouds of several million points at a stable framerate in any compatible VR headset. 
...
\textbf{sidebar.html}
\begin{lstlisting}
    <li><label><input type="checkbox" onClick="viewer.setVREnabled(true)"/>\\
    <span data-i18n="appearance.vr">\\
    </span></label></li>
\end{lstlisting}
...
\textbf{Potree.JS LINE 13250 - 13286}

\begin{verbatim}
   setVREnabled(value){
        if(this.useVR != value){
            this.useVR = value;
            this.dispatchEvent({"type": "use_vr_changed", "viewer": this});
        }
        if(this.useVR)
        {
...
\textbf{Potree.JS LINE 17894 - 17908}
